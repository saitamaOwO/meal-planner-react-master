...
export default function App({ Component, pageProps }: AppProps) {
  ...
  const [pcalories, setPcalories] = useState<number>(0);
  useEffect(() => {
    ...
    axiosInstance
      ...
      .catch((error) => {
        console.error({ error });
        alert("Something went wrong, profile not found");
      })
      .then((res) => {
        setPcalories(res?.data?.payload?.pcalories);
      });
  }, []);
          ...
          {username && (
            <>
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-xs">Intake/day : {pcalories}</p>
              </div>
              <button
                onClick={signOut}
                className="border px-4 py-1.5 h-fit border-zinc-700 rounded-lg text-sm hover:bg-zinc-700 transition-all"
              >
                Sign Out
              </button>
            </>
